...
↑ をコンパイルする状況を想定する (main の実行結果は 120)
;)
...
(module
  ...
  (func (export "main") (result i32)
    ...
    call $push
    (i32.store (call $top) (i32.const 5))
    (call $factorial (i32.load (call $top)))
    call $pop
    call $free) ;; => 120

  (func $factorial (param i32) (result i32)
    (if (result i32) (i32.eqz (i32.gt_s (get_local 0) (i32.const 0)))
      (then
        (i32.const 1))
      (else
        (i32.mul
          (get_local 0)
          (call $factorial (i32.sub (get_local 0) (i32.const 1))))))))
